...
public class Utils {
	...
	public static void alert_fshirje2(JFXAlert alert, Button konfirmo,JFXButton anullo, boolean overlay, String imagePath) {
		alert.initModality(Modality.APPLICATION_MODAL);
		alert.setOverlayClose(overlay);
		JFXDialogLayout layout = new JFXDialogLayout();

		if(overlay == false) {
			layout.setBody(new Label("KUJDES! Shoqerohet me fshirje te dhenash! KUJDES! KUJDES! KUJDES! KUJDES! KUJDES!"));
			layout.setActions(konfirmo,anullo);
		}
		else {
			ImageView img = new ImageView(new Image(imagePath));
        ...
        change_pass.setOnAction(e -> {
        	 ...
        	 try {
				if(!ControlDAO.getControlDao().getLoginDao().get_pass().equals(existing_password.getText().toString())) {
					lblError.setText("Passwordi i vjeter nuk eshte i sakte!");
					return;
				}
     		...
     		if (!new_password.getText().toString().equals(repeat_new_password.getText().toString())) {
     			lblError.setText("Passwordi i ri nuk eshte njesoj!");
     			return;
     		}
