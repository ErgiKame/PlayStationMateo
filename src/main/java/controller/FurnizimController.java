...
public class FurnizimController extends VBox {
	...
	@FXML
	private TableColumn<Furnizim, String> tblColFurnizimId, tblColArtikulli, tblColSasia, tblColCmimi, tblColVlera,
	tblColData;
	@FXML
	...
	private void loadFurnizim() throws SQLException {
		...
		tblColDelete.setCellFactory(param -> new TableCell<Furnizim, Furnizim>() {
			...
			protected void updateItem(Furnizim furnizim, boolean empty) {
				super.updateItem(furnizim, empty);
				...
				delete.setOnMouseClicked(event -> {
					...
					konfirmo.setOnAction(e -> {
						delete(furnizim.getId(), furnizim.getArtikull_id(), furnizim.getSasia());
						alert.close();
					});
	...
	private void getData() throws IOException, SQLException {
		...
		furnizimDataHolder.setArtikull_id(furnizim.getArtikull_id());
	...
	private void delete(int furnizimId, Artikujt artikujt, double sasia) {
		try {
	...
	private void excel() {
		try {
			...
			for (int i = 0; i < tableViewData.size(); i++) {
				text = i + 1 + "," + tableViewData.get(i).getArtikull_id().getEmri_artikullit() + ","
