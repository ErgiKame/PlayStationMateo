...
public class PerdoruesitController extends VBox {
	...
	@FXML
	private TableColumn<Perdoruesit, String> tblColPerdoruesi, tblColEmri, tblColTeDrejtat, tblColTelefon, tblColUserid;
	...
	private void excel() {
		try {
			...
			for(int i=0; i<tableViewData.size(); i++){
				...
				text = i+1 + "," + tableViewData.get(i).getUsername()+ "," + tableViewData.get(i).getName()+ "," 
				+ tableViewData.get(i).getAccess() + "," + tableViewData.get(i).getTelefon()+ "\n";
	...
	private void pdf() {
		try {
			...
			t.addCell(c2);
			Phrase phrase3 = new Phrase("Te drejtat", bold);
			PdfPCell c3 = new PdfPCell(phrase3);
			c3.setHorizontalAlignment(Element.ALIGN_CENTER);
			t.addCell(c3);
			Phrase phrase4 = new Phrase("Telefon", bold);
